function edit_task(task_id) {
    ...
    td_description.innerHTML = "<input id ='input_description" + task_id + "' type = 'text' value= '" + td_description.innerHTML + "'>";

    let td_status = children[2];
    let status_id = "#select_status_" + task_id;
    let status_current_value = td_status.innerHTML;
    td_status.innerHTML = getDropdownStatus(task_id);
    $(status_id).val(status_current_value).change();

}

function getDropdownStatus(task_id) {
    let status_id = "select_status_" + task_id;
...
function add_task(){
    let value_description = $("#description_name").val();
